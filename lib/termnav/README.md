...
single installed `termnav` command.

## Areas

- `nvim-open/launcher.sh` is the small sourceable shell predicate that reuses
  an editor pane only for a simple file open typed at an interactive prompt;
  all routing and transport policy remains in the Rust binary.
